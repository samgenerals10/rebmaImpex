...
export default function Sidebar({
  ...
  return (
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 app-sidebar flex flex-col justify-between shrink-0 shadow-lg select-none transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static bg-slate-900 text-slate-300`}>
      {/* 1. MOBILE SLACK STYLE VIEW */}
      <div className="lg:hidden flex flex-col h-full justify-between py-6 px-4 bg-slate-950 text-slate-300">
        <div className="flex flex-col h-full overflow-hidden">
          ...
          <div className="flex items-center justify-between pb-4 border-b border-slate-800">
            <div className="flex items-center gap-2">
              ...
              />
              <div>
                <h2 className="font-extrabold text-xs tracking-wider text-white leading-tight">REBMA IMPEX</h2>
                <span className="text-[9px] uppercase text-emerald-400 font-mono tracking-widest leading-none">GHANA</span>
              </div>
            </div>
            {onClose && (
              <button 
          ...
          <div className="py-4 flex items-center gap-2.5 border-b border-slate-800 shrink-0">
            <div className="w-9 h-9 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-sm shrink-0 relative">
              {currentUser?.fullName?.[0] || 'U'}
              <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-slate-950" />
            </div>
            <div className="truncate flex-1">
              <p className="text-xs font-bold text-white leading-none truncate">{currentUser?.fullName}</p>
              <p className="text-[10px] text-slate-400 leading-none mt-1 truncate">{currentUser?.department}</p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pt-4 space-y-4">
            <div>
              <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest px-2 mb-1.5">Channels</p>
                ...
                {availableDepts.map(dept => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center px-2 py-2 rounded text-xs font-medium transition-colors text-left truncate ${
                        isSelected 
                          ? 'bg-slate-800 border-l-4 border-emerald-500 text-white font-bold pl-1.5' 
        ...
        {/* Bottom Section */}
        <div className="pt-4 border-t border-slate-850 flex flex-col gap-2 shrink-0">
          <button
            onClick={() => { setActiveDepartment('SETTINGS'); setActiveSubTab('Themes'); onClose?.(); }}
            className={`flex items-center gap-2.5 px-3 py-2 rounded text-xs font-medium transition-colors ${
              activeDepartment === 'SETTINGS' ? 'bg-slate-800 text-white font-bold' : 'text-slate-400 hover:bg-slate-900 hover:text-slate-200'
            }`}
          >
          ...
          <button
            onClick={onLogout}
            className="flex items-center gap-2.5 px-3 py-2 rounded text-xs font-medium text-rose-400 hover:bg-rose-950/20 hover:text-rose-300 transition-colors"
          >
