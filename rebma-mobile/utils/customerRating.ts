// rebma-mobile/utils/customerRating.ts
//
// Phase 7.3, D21. Ported from rebma-web/src/utils/customerRating.ts —
// same algorithm, thresholds, and reasoning (comments kept), only adapted
// from camelCase (o.totalAmount) to snake_case (o.total_amount) field
// access, matching mobile's established no-mapper-layer convention: every
// mobile screen reads raw Supabase rows directly, unlike web's
// mapOrderToFrontend()/mapCustomerToFrontend() layer.
//
...
export function ordersForCustomer(orders: OrderLike[], customerName: string): OrderLike[] {
  return orders.filter(o => sameCustomerName(o.client_name || '', customerName));
}
...
export function ordersForCustomerRow(orders: OrderLike[], customer: CustomerLike): OrderLike[] {
  return orders.filter(o => o.customer_id ? o.customer_id === customer.id : sameCustomerName(o.client_name || '', customer.name));
}

// Unpaid portion of a customer's live credit orders — mirrors the RPC's
// definition exactly: driven by amount_paid, not status (a DELIVERED credit
// order is delivered, not paid), excluding orders that never became real
// exposure (REJECTED/CANCELLED/RETURNED_FOR_CORRECTION).
export function outstandingCreditFor(orders: OrderLike[], customer: CustomerLike): number {
  return ordersForCustomerRow(orders, customer)
    .filter(o => (o.payment_mode || '').toUpperCase() === 'CREDIT')
    .filter(o => !['REJECTED', 'CANCELLED', 'RETURNED_FOR_CORRECTION'].includes(o.status || ''))
    .reduce((sum, o) => sum + Math.max((o.total_amount || 0) - (o.amount_paid || 0), 0), 0);
}
...
export function computeCustomerRating(customerOrders: OrderLike[]): CustomerRating {
  const counted = customerOrders.filter(o => o.status !== 'REJECTED');
  const orderCount = counted.length;
  const totalPurchased = counted.reduce((s, o) => s + (Number(o.total_amount) || 0), 0);
  ...
  if (orderCount >= 2) {
    const firstOrderMs = Math.min(...counted.map(o => new Date(o.created_at || 0).getTime()));
...
// input, so a high score translates into a concrete number instead of
// Management having to guess what a given grade "should" be worth.
